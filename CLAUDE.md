...
- Home画面フィード表示件数は最大12件(`MAX_DISPLAYED_ARRIVALS`、到着時刻昇順。2026-09-20ユーザー指示で10→12に変更、3列グリッドで割り切れる件数にするため)
